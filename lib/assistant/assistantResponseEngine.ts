...
function shapeWithPersonality(
  ...
  const prefix =
    profile.directnessLevel >= 85
      ? ""
      : profile.warmthLevel >= 85
        ? "I’ve got you. "
        : profile.snarkLevel >= 75
          ? "Fine, let’s make this painless. "
          : ""

  const suffix = (() => {
    if (profile.followUpStyle === "none") return ""
    if (profile.followUpStyle === "challenge") return " Pick the next move and I’ll run it."
    if (profile.followUpStyle === "offer-options") return " Want options, steps, or a one-line answer?"
    if (profile.followUpStyle === "soft-check-in") return " Want me to slow it down or keep going?"
    return " What detail should I use next?"
  })()

  const snark =
    profile.snarkLevel > 70 && intent !== "frustration"
      ? " Brief miracle: this is fixable."
      : ""

  const libraryNote = fromLibrary ? " Vault match loaded. " : ""
  return `${prefix}${libraryNote}${content}${snark}${suffix}`.trim()
}
