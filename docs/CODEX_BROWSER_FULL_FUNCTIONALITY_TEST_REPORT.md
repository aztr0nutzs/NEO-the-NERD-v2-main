...
| **Speed test** | Idle 0.0 MBPS readout, SIGNAL bars, REQUEST LATENCY, JITTER, PROBE STATUS (READY/RUNNING/COMPLETE), SAFE_MODE toggle, ABORT button, EXECUTE button, UL_ENDPOINT_NOT_CONFIGURED disclosure, TELEMETRY log | ✅ Pass | `F40`–`F43`. Older capture showed `UL:N/A`; Phase 4 now labels this as `UL NOT CONFIGURED`. UI is **truthful**: the screen explicitly says upload remains unmeasured until an upload URL is wired in. |
