...
| 12 | **Network demo/live badges truthful** | `truthBadge` in `NetworkMap3D` resolves to `DEMO TOPOLOGY VIEW`, `ESTIMATED LOGICAL TOPOLOGY`, or `BACKEND CONFIRMED TOPOLOGY`. Scan mode copy now matches the native implementation: Quick uses context/gateway/shallow TCP probes; Balanced adds ARP/MAC, hostname, SSDP, and bounded ports; Deep expands host/timeout/port caps. No mDNS or full port scan is claimed. | Manual device confirmation remains required. |
