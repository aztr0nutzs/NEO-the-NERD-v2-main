...
| 3 | **Avatar not boxed** | `NeoAvatarVideo` uses a stage-specific elliptical mask + crop tuning (`variant="stage"`) and `RobotStage` provides a deeper radial reactor backing so the avatar blends into a portal-like chamber instead of reading as a rectangular media block. | Manual — user must confirm the robot reads as integrated into the stage, not as a square. |
