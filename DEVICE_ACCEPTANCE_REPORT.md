...
**Date (UTC):** 2026-05-14
**Branch verified:** `main` at commit `dd930fe`
**Verifier environment:** Linux sandbox without Android device, emulator, ADB, or any APK-installation surface.
...
| 3 | **Avatar not boxed** | `NeoAvatarVideo` accepts `portalMode` prop which applies `mix-blend-mode: screen` and a radial `mask-image` to the inner `<video>`, eliminating the MP4 black rectangle. `RobotStage` passes `portalMode` and renders a dark radial "reactor portal" backing behind the avatar (PR #10). | Manual — user must confirm the robot reads as integrated into the stage, not as a square. |
...
| 6 | **Voice Library previews work** | `previewVoice({ mode: "auto" })` picks provider-TTS if reachable, otherwise browser SpeechSynthesis (which on Capacitor Android surfaces the system TTS engine). Stop/pause/resume wired. Truth label switches between *Connected / Local Fallback / Backend Unavailable / Detecting*. (`lib/voice/voice-runtime.ts`, `components/screens/voices-screen.tsx`) | Manual — user must preview ≥3 voices and confirm audible output or the truthful unavailability label. |
...
| 12 | **Network demo/live badges truthful** | `truthBadge` in `NetworkMap3D` resolves to `DEMO TOPOLOGY VIEW`, `ESTIMATED LOGICAL TOPOLOGY`, or `BACKEND CONFIRMED TOPOLOGY` based on `topologyGraph.topologyMode`. `effectiveAdapterStatus` in `NetworkDiscoveryFeature` shows `DEMO_ADAPTER` / `LIVE_ANDROID_DISCOVERY` / `BACKEND_CONNECTED` truthfully. | Manual — user must confirm the badge matches actual mode. |
