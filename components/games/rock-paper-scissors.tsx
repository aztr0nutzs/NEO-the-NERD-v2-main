...
export function RockPaperScissorsGame({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    ...
    recordGameResult({ game: "rps", result: score.you > score.bot ? "win" : "lose", difficulty: settings.gameDifficulty, score: score.you, streak: bestStreak })
  }, [done, score.you, score.bot, bestStreak, recordGameResult, settings.gameDifficulty])

  const resetMatch = () => {
    setYou(null); setBot(null); setRevealing(false)
    setScore({ you: 0, bot: 0 }); setHistory([]); setStreak(0); setBestStreak(0)
    setStatusOverride(null); recordedRef.current = false
  }

  const status = revealing
    ? "NEO PREDICTION MATRIX RUNNING…"
    : done ? (score.you > score.bot ? "MATCH WON." : "MATCH LOST.")
    : statusOverride ?? (result === "win" ? "YOU WIN ROUND." : result === "lose" ? "NEO TAKES ROUND." : result === "draw" ? "DRAW." : "PICK YOUR HAND")

  return <NeonPanel accent="pink" glow="strong" className="p-3"><div className="flex items-center justify-between"><div><p className="ps-mono text-[10px] tracking-[0.3em] ps-text-pink">ROCK_PAPER_SCISSORS // FIRST_TO_{target}</p><p className="ps-mono text-[10px] tracking-[0.25em] text-white/65">{status}</p><p className="ps-mono text-[9px] tracking-[0.2em] text-white/55">STREAK {streak} · BEST {bestStreak}</p></div><div className="flex items-center gap-2"><ScorePill label="YOU" value={score.you} color="#00f0ff" /><ScorePill label="NEO" value={score.bot} color="#ff2d9c" />{onClose && <button type="button" onClick={onClose} className="rounded-lg px-3 py-2 ps-mono text-[10px] tracking-[0.25em] text-white/85" style={{ boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.15)" }}>EXIT</button>}</div></div>
  ...
  {done && <div className="mt-3 rounded-lg bg-black/50 px-3 py-2"><p className="ps-mono text-[10px] tracking-[0.25em] text-white/80">{score.you > score.bot ? "MATCH WON" : "MATCH LOST"} · {score.you} – {score.bot}</p><p className="mt-1 ps-mono text-[9px] tracking-[0.2em] text-white/55">BEST STREAK {bestStreak} · DIFFICULTY {settings.gameDifficulty}</p><button type="button" onClick={resetMatch} className="mt-2 w-full rounded-lg py-2 ps-mono text-[11px] tracking-[0.25em]" style={{background:"rgba(255,45,156,0.16)", color:"#ff2d9c", boxShadow:"inset 0 0 0 1px rgba(255,45,156,0.5)"}}>NEW MATCH</button></div>}
  </NeonPanel>
}
